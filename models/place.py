...
class PlaceModel(db.Model):
	...
	def json(self):
		return {'name': self.name, 'desc_sv': self.desc_sv, 'desc_en': self.desc_en, 'desc_de': self.desc_de, 'lat': self.lat, 'lon': self.lon, 'images': [images.json() for image in self.images.all()]}
